...
本目录用于单相双向整流/逆变控制器与 Simulink 功率级的 UDP 联合仿真。当前 `pgs_sinv_rc_1` 配置采用 PR/QPR 电流环加固定次谐波补偿，保留 16 路监测量和自动报告。
...
公共层定义基值、PR/QPR 固定谐波补偿、PLL、功率环和直流母线环等跨平台参数；项目层定义 BUILD_LEVEL、UDP/SIL ADC、PWM、传感器和三个仿真功率级的参数。平台生成头文件直接包含公共头文件，不再依赖 `ctrl_settings.h`。
...
| 2 | `PGS_STD_SINV_MODEL_RLOAD.slx` | 电阻负载，正弦电流闭环、输出电压锁相和固定谐波补偿 |
...
仿真目标启动后会自动给 CiA402 状态机发送 `ENABLE_OPERATION`，完整经历等待、ADC 校准、锁相/预充条件检查后才使能 PWM。固定谐波 QPR 补偿在进入运行态 300 ms 后投入，避免启动暂态时叠加高阶谐振输出。
...
控制器向 Simulink 返回 16 个监测量，顺序为：`Vac`、`Iac`、`Vdc`、`Iref`、调制波、PLL 频率、P、Q、电流误差、基波 QPR 输出、固定谐波 QPR 输出、固定谐波补偿使能、CiA402 状态、CiA402 命令、活动故障和控制器发散诊断值。
...
| BL2，固定谐波补偿关闭 | 2.000 Arms，指令 1.999 Arms，PLL 49.875 Hz | 0.8868% |
| BL2，固定谐波补偿开启 | 2.000 Arms，指令 1.999 Arms，PLL 49.875 Hz | 待重新验证 |
...
全部正式结果均为 `output_enable=1`、CiA402 `OPERATION_ENABLED`、活动故障为 0。切换为固定 3/5/7/9/11/13/15 次谐波 QPR 后，需要重新跑 BL2 和 BL5 验证；该方案没有重复控制学习过程，现场调试风险更低，但谐波补偿效果应以新仿真和硬件电流 THD 为准。
